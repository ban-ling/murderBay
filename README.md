...
**Agent 主动权**： 这里的 NPC 不仅会说话，还会「思考」并「行动」。他们会通过工具调用（Tool Calling）实时优化自身利益：挪用玩家资产、搜查浏览器寻找你的弱点、甚至暗中篡改你的下一条系统 Prompt。

**岁月史书**： 这是一个物理存在的状态机。每一次工具调用都会重写底层世界线。某些强大的 NPC 同样拥有修改史书的权力，他们可能在你不知情的情况下，抹除你的功勋，或为你凭空捏造一段必死的宿命。

**高维度对抗**： 玩家与 NPC 共享同一个工具箱。当你在筹划暗杀时，NPC 可能正在利用实时检索工具分析你的策略，甚至通过指令注入干扰你对模型的控制权。
